...
public class CalculatorFragment extends Fragment implements OnSeekBarChangeListener, OnClickListener {
	...
	// Input tags
	private final String[] INPUTS = {"chlorine", "ph", "alkalinity", "hardness", "stabilizer", "temperature"};
	...
	public void onCreate(Bundle savedState) {
		...
		mValues = new HashMap<>();
		mCurrentValues = new HashMap<>();
		mDefaultValues = new HashMap<>();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
		...
		inputs[0] = v.findViewById(R.id.chlorine);
		inputs[1] = v.findViewById(R.id.ph);
		inputs[2] = v.findViewById(R.id.alkalinity);
		inputs[3] = v.findViewById(R.id.hardness);
		inputs[4] = v.findViewById(R.id.stabilizer);
		inputs[5] = v.findViewById(R.id.temperature);
		...
		for (int i = 0; i < inputs.length; i++) {
			...
			((TextView) inputs[i].getChildAt(1)).setText((current_value >= 0) ? values[current_value] : "- - -");
		}
		...
		if (savedState == null) {
			...
			mSelectedInput = v.findViewById(R.id.chlorine);
			mSelectedInput.setSelected(true);
			tag = TAG_CHLORINE;
		} else {
			mSelectedInput = v.findViewById(savedState.getInt(TAG_SELECTED_INPUT));
			mSelectedInput.setSelected(true);
		...
		mClear.setOnClickListener(v1 -> clearValue());
