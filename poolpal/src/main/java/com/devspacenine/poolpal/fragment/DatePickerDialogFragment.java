...
import com.devspacenine.poolpal.widget.labeler.*;
...
public class DatePickerDialogFragment extends InputDialogFragment {
	...
	public void onCreate(Bundle savedState) {
		...
		if (!mValues.containsKey(TaskDataAdapter.VALUE_DATE) || mValues.getSerializable(TaskDataAdapter.VALUE_DATE) == null) {
			mInitDate = null;
		...
		if (!mValues.containsKey(TaskDataAdapter.VALUE_DATE_INTERVAL) || mValues.getInt(TaskDataAdapter.VALUE_DATE_INTERVAL) < 1) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
		...
		mCancelButton.setOnClickListener(v12 -> negativeDecision());

		mConfirmButton.setOnClickListener(v1 -> positiveDecision());
	...
	private OnTimeChangeListener onTimeChangeListener = time -> setTitle();
		...
		if (mTitleText != null) {
			...
			switch (getArguments().getInt(LAYOUT)) {
				case R.layout.date_slider:
					mTitleText.setText(mCtx.getString(R.string.dateSliderTitle) + String.format(Locale.getDefault(), ": %te. %tB %tY", c, c, c));
					break;

				case R.layout.month_year_date_slider:
					mTitleText.setText(mCtx.getString(R.string.dateSliderTitle) + String.format(Locale.getDefault(), ": %tB %tY", c, c));
					break;
				...
				case R.layout.date_time_slider:
					...
					mTitleText.setText(String.format(Locale.getDefault(), "Selected DateTime: %te/%tm/%ty %tH:%02d", c, c, c, c, minute));
					break;
