...
public class EditPoolActivity extends /*Sherlock*/AppCompatActivity implements OnDecisionListener, LoaderManager.LoaderCallbacks<Cursor>, TempDataListener {
	...
	public void onCreate(Bundle savedState) {
		...
		if (mLayout == R.layout.init_pool) {
			mListView = findViewById(R.id.settings);
			...
			mTabsAdapter.addTab(res.getString(R.string.btn_info), ProfileTabsAdapter.TAB_INFO, PoolInfoFragment.class, mTabWidget);
			mTabsAdapter.addTab(res.getString(R.string.btn_tasks), ProfileTabsAdapter.TAB_TASKS, PoolTasksFragment.class, mTabWidget);
			mTabsAdapter.addTab(res.getString(R.string.btn_charts), ProfileTabsAdapter.TAB_CHARTS, ChartsFragment.class, mTabWidget);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
			case PoolPal.SET_IMAGE:
				if (resultCode == /*Sherlock*/AppCompatActivity.RESULT_OK) {
					try {
						...
						if (data.hasExtra("data")) {
							...
							if (mTempData.containsKey(PoolPal.EXTRA_IMAGE_URI)) {
								...
								throw new IOException("Returning Activity/Fragment must save the captured " + "image URI with the TempDataListener interface.");
							}
						} else {
	...
	public void setImage(View v) {
		...
		if (getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA)) {
			...
			args.putString(InputDialogFragment.TITLE, getResources().getString(R.string.title_set_image));
			args.putString(InputDialogFragment.DETAILS, getResources().getString(R.string.prompt_set_image));
	...
	public void populate(int page) {
		...
		if (!mPopulated) {
			...
			ImageView health_icon = findViewById(R.id.health_icon);
			if (health_icon != null) {
			...
			switch (page) {
				case R.layout.init_pool:
					LinkedList<Bundle> basic_data = new LinkedList<>();

					basic_data.add(PoolDataAdapter.createItem(res.getString(R.string.lbl_name), res.getString(R.string.details_name), PoolDataAdapter.ACTION_SET, PoolDataAdapter.SET_NAME, mPool.hasName()));
					basic_data.add(PoolDataAdapter.createItem(res.getString(R.string.lbl_dimensions), res.getString(R.string.details_dimensions), PoolDataAdapter.ACTION_SET, PoolDataAdapter.SET_DIMENSIONS, mPool.hasDimensions()));
					basic_data.add(PoolDataAdapter.createItem(res.getString(R.string.lbl_traffic), res.getString(R.string.details_traffic), PoolDataAdapter.ACTION_SET, PoolDataAdapter.SET_TRAFFIC, mPool.hasTraffic()));
					basic_data.add(PoolDataAdapter.createItem(res.getString(R.string.lbl_address), res.getString(R.string.details_address), PoolDataAdapter.ACTION_SET, PoolDataAdapter.SET_ADDRESS, mPool.hasAddress()));

					mAdapter = new SettingsSectionAdapter(this, mPool);
					mAdapter.addSection(res.getString(R.string.title_pool_details), new PoolDataAdapter(this, mPool, basic_data, R.layout.edit_settings_item));
	...
	public void onPositiveDecision(int requestCode, Bundle values) {
		...
		if (mLayout == R.layout.init_pool) {
			...
			if (mAdapter.allItemsSet()) {
				...
				TutorialFragment frag = TutorialFragment.newInstance(TutorialFragment.CONTINUE_EDITING, R.layout.tutorial_dialog, data, null);
