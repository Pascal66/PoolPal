...
public class ScrollLayout extends LinearLayout {
	...
	public ScrollLayout(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ScrollLayout, 0, 0);
		...
		// values as defaults
		objWidth = a.getDimensionPixelSize(R.styleable.ScrollLayout_childWidth, mLabeler.getPreferredViewWidth(context));
		objHeight = a.getDimensionPixelSize(R.styleable.ScrollLayout_childHeight, mLabeler.getPreferredViewHeight(context));
	...
	protected void onFinishInflate() {
		...
		int childCount = dimensions.x / objWidth;
		// Make sure to round up
		if (dimensions.x % objWidth != 0) {
			childCount++;
		}
	...
	protected void reScrollTo(int x, int y, boolean notify) {
		if (notify) Log.d(TAG, "scroll to " + x);
		...
		if (listener != null && notify) {
			...
			if (notify) Log.d(TAG, "real time " + currentTime);
			if (notify) Log.d(TAG, "");
