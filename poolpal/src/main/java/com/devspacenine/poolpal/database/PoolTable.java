...
public class PoolTable {
	...
	private static final String DATABASE_CREATE = "create table " + TABLE + " (_id integer primary key autoincrement, address_id integer default 0, name text not null default \'My Pool\', " + "volume real not null default -1, pool_locale text, finish text, sanitizer text, " + "pump_brand text, pump_model text, filter text, cleaner_brand text, cleaner_model text, " + "traffic text not null default \'Private - Family & Friends\', min_depth real not null default 3, max_depth real not null default 9, " + "tiling integer not null default 0, cover integer not null default 0, " + "attached_spa integer not null default 0, heater integer not null default 0, diving_board integer not null default 0, " + "slide integer not null default 0, ladder integer not null default 0, fountains integer not null default 0, " + "rock_waterfall integer not null default 0, lights integer not null default 0, infinity integer not null default 0, " + "sporting_equipment integer not null default 0, beach_entry integer not null default 0, " + "sand integer not null default 0, image text, weather_notifications integer not null default 1, " + "water_test_reminders integer not null default 1, filter_reminders integer not null default 1, " + "safety_notifications integer not null default 1, maintenance_reminders integer not null default 1, " + "custom_notifications integer not null default 1, coupon_notifications integer not null default 1);";
	...
	public static void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
		...
		Log.w(PoolTable.class.getName(), "Upgrading database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
