...
public class PoolDataAdapter extends DataAdapter {
	...
	public PoolDataAdapter(FragmentActivity context, Pool pool, LinkedList<Bundle> data, int resource) {
		super(context, pool, data, resource);
	}

	public PoolDataAdapter(FragmentActivity context, Pool pool, LinkedList<Bundle> data, int resource, String fragmentTag) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (action.equals(ACTION_TOGGLE_SMALL)) {
			if (convertView == null) {
				...
				TableLayout table = v.findViewById(R.id.toggle_items);
				String[] options = data.getStringArray(ITEM_LABELS);
				int rows = (options.length / 2) + (options.length % 2);
				for (int i = 0; i < rows; i++) {
					TableRow tr = (TableRow) inflater.inflate(R.layout.edit_settings_table_row, table, false);
			...
			RelativeLayout buttonView = v.findViewById(R.id.button);
			TextView titleView = buttonView.findViewById(R.id.title);
			TextView valueView = buttonView.findViewById(R.id.value);
			...
			if (valueView instanceof CheckBox) {
				...
				TextView detailsView = buttonView.findViewById(R.id.details);
			...
			if (data.containsKey(ITEM_SET)) {
				...
				if (data.getString(ITEM_ACTION).equals(ACTION_SET)) {
					ImageView set = v.findViewById(R.id.set);
			...
			buttonView.setOnClickListener(v1 -> {
				...
				String _action = v1.getTag(R.id.key_action).toString();
				String _title = (String) v1.getTag(R.id.key_title);
				String _details = (String) v1.getTag(R.id.key_details);
				int requestCode = (Integer) v1.getTag(R.id.key_request_code);
				...
				if (_action.equals(ACTION_EDIT) || _action.equals(ACTION_SET)) {
					...
					Bundle args = InputDialogFragment.createItem(requestCode, _title, _details, mLayouts.get(requestCode), values, mFragmentTag);
					...
					Bundle args = TaskSettingsFragment.createItem(requestCode, mPool, mFragmentTag);
					((EditPoolActivity) mCtx).replaceTab(ProfileTabsAdapter.TAB_GENERAL_MAINTENANCE, TaskSettingsFragment.class, args);

				} else if (_action.equals(ACTION_TOGGLE)) {

					// This list item toggles a value
					((CheckBox) v1.findViewById(R.id.value)).toggle();
	...
	private void populateColumn(View col, String[] options, int pos) {
		...
		col.setId(pos);
		CheckBox value = col.findViewWithTag(res.getString(R.string.value));
		TextView label = col.findViewWithTag(res.getString(R.string.label));
		...
		col.setOnClickListener(v -> {
			CheckBox val = v.findViewWithTag(mCtx.getResources().getString(R.string.value));
			val.toggle();
