...
public class Treatment implements Parcelable {
	...
	public static ArrayList<Treatment> listFromString(Context ctx, String s) throws ParseException {
		ArrayList<Treatment> l = new ArrayList<>();
		String[] _ids = s.split(",");
		for (String _id : _ids) {
	...
	public Treatment(Context context, long id) throws ParseException {
		this(context, context.getContentResolver().query(Uri.withAppendedPath(PoolPalContent.TREATMENTS_CONTENT_URI, Long.toString(id)), TreatmentTable.columnProjection(), null, null, null));
	}
	...
	public void writeToParcel(Parcel out, int flags) {
		...
		out.writeBooleanArray(new boolean[]{netRaked, skimmerCleaned, pumpBasketCleaned, cleanerEmptied, vacuumed, brushed, backwashed, filterWashed, drained, waterAdded});
	}
