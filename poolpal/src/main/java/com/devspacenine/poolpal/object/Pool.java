...
public class Pool implements Parcelable {
	...
	public static ArrayList<Pool> listFromString(Context ctx, String s) throws ParseException {
		ArrayList<Pool> l = new ArrayList<>();
		String[] _ids = s.split(",");
		for (String _id : _ids) {
	...
	public Pool(Context context, long id) {
		this(context, context.getContentResolver().query(Uri.withAppendedPath(PoolPalContent.POOLS_CONTENT_URI, Long.toString(id)), PoolTable.columnProjection(), null, null, null));
	}
	...
	public Pool(Context context, Cursor cursor) {
		...
		if (cols.containsKey(PoolTable.KEY_ADDRESS)) {
			this.address = new PoolAddress(context, cursor.getLong(cols.get(PoolTable.KEY_ADDRESS)));
		} else {
	...
	public boolean[] getAccessories() {
		return new boolean[]{tiling, cover, attachedSpa, heater, divingBoard, slide, ladder, fountains, rockWaterfall, lights, infinity, sportingEquipment, beachEntry, sand};
	}
	...
	public Bundle getAddressBundle() {
		...
		Bundle b = new Bundle();
		b.putParcelable(PoolDataAdapter.VALUE_ADDRESS, (hasAddress()) ? PoolAddress.setLines(address) : PoolAddress.blankAddress());
	...
	public String getVolumeString() {
		if (volume > 0) {
			return String.format(Locale.getDefault(), "%1$,.0f g", volume);
		} else {
			return "";
		}
	}
	...
	public Bundle getDimensionsBundle() {
		...
		b.putString(PoolDataAdapter.VALUE, !hasDimensions() ? "" : String.format(Locale.getDefault(), "%1$,.0f ft - %2$,.0f ft\n%3$s", minDepth, maxDepth, getVolumeString()));
		return b;
	}
	...
	public String getDepth() {
		if (minDepth < 0 || maxDepth < 0) {
			...
			return String.format(Locale.getDefault(), "%1$f ft - %2$f ft", minDepth, maxDepth);
		}
	}
	...
	/**
	 * @param maintenanceReminders the cleaningReminders to set
	 */
	...
	public void writeToParcel(Parcel out, int flags) {
		...
		out.writeBooleanArray(new boolean[]{tiling, cover, attachedSpa, heater, divingBoard, slide, ladder, fountains, rockWaterfall, lights, infinity, sportingEquipment, beachEntry, sand, weatherNotifications, waterTestReminders, filterReminders, safetyNotifications, maintenanceReminders, customNotifications, couponNotifications});
	}
