...
public class WaterTest implements Parcelable {
	...
	public static ArrayList<WaterTest> listFromString(Context ctx, String s) throws ParseException {
		ArrayList<WaterTest> l = new ArrayList<>();
		String[] _ids = s.split(",");
		for (String _id : _ids) {
	...
	public WaterTest(Context context, long id) throws ParseException {
		this(context, context.getContentResolver().query(Uri.withAppendedPath(PoolPalContent.WATER_TESTS_CONTENT_URI, Long.toString(id)), WaterTestTable.columnProjection(), null, null, null));
	}
	...
	public double saturationPH() {
		...
		if (temperature < 0) {
			throw new IllegalStateException("Temperature required to calculate saturation pH.");
		}
		if (hardness < 0) {
			throw new IllegalStateException("Hardness required to calculate saturation pH.");
		}
		if (alkalinity < 0) {
			throw new IllegalStateException("Alkalinity required to calculate saturation pH.");
		}

		double logKs = -171.9065d - (0.077993d * temperature) + (2839.319d / temperature) + (71.595d * Math.log10(temperature));
		double logK2 = -107.8871d - (0.032528d * temperature) + (5151.79d / temperature) + (38.92561d * Math.log10(temperature)) - (563713.9d / Math.pow(temperature, 2d));
		double mu = 0.000025d * ((double) tds);
		double eps = (60954d / (temperature + 116d)) - 68.937d;
		double logyCa = -(1825000d * (Math.pow((eps * temperature), -1.5d))) * 4d * ((Math.sqrt(mu) / (1d + Math.sqrt(mu))) - (0.3d * mu));
		double logyHCO3 = -(1825000d * (Math.pow((eps * temperature), -1.5d))) * ((Math.sqrt(mu) / (1d + Math.sqrt(mu))) - (0.3d * mu));
