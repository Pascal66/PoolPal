...
public class PoolAddress extends Address implements Parcelable {
	...
	public static ArrayList<PoolAddress> listFromString(Context ctx, String s) throws ParseException {
		ArrayList<PoolAddress> l = new ArrayList<>();
		String[] _ids = s.split(",");
		for (String _id : _ids) {
	...
	public static boolean equals(Address a, Address b) {
		return PoolAddress.toString(PoolAddress.setLines(a)).equals(PoolAddress.toString(PoolAddress.setLines(b)));
	}

	public static boolean isComplete(Address a) {
		return a != null && (a.getFeatureName() != null && a.getThoroughfare() != null && a.getLocality() != null && a.getAdminArea() != null && a.getPostalCode() != null);
	}
	...
	public static String toString(Address a) {
		return String.format("%1$s%2$s%3$s", (a.getMaxAddressLineIndex() >= 0) ? a.getAddressLine(0) : "", (a.getMaxAddressLineIndex() >= 1) ? "\n" + a.getAddressLine(1) : "", (a.getMaxAddressLineIndex() >= 2) ? "\n" + a.getAddressLine(2) : "");
	}
	...
	public static Address setLines(Address a) {
		...
		sb.append((a.getAdminArea() != null && a.getPostalCode() != null) ? ", " + a.getPostalCode() : (a.getPostalCode() != null) ? " " + a.getPostalCode() : "");
		a.setAddressLine(1, sb.toString());

		// Line 3
		sb = new StringBuilder();
		sb.append((a.getCountryCode() != null) ? a.getCountryCode() : (a.getCountryName() != null) ? a.getCountryName() : "");
	...
	public static boolean isBlank(Address a) {
		String val = String.format("%1$s%2$s%3$s%4$s%5$s", (a.getMaxAddressLineIndex() >= 0) ? a.getAddressLine(0) : "", (a.getMaxAddressLineIndex() >= 1) ? a.getAddressLine(1) : "", (a.getMaxAddressLineIndex() >= 2) ? a.getAddressLine(2) : "", (a.getMaxAddressLineIndex() >= 3) ? a.getAddressLine(3) : "", (a.getMaxAddressLineIndex() >= 4) ? a.getAddressLine(4) : "");
		return val.equals("");
	}
	...
	public PoolAddress(Context context, long id) {
		this(context, context.getContentResolver().query(Uri.withAppendedPath(PoolPalContent.ADDRESSES_CONTENT_URI, Long.toString(id)), AddressTable.columnProjection(), null, null, null));
	}
	...
	public String toString() {
		return String.format("%1$s%2$s%3$s", (getMaxAddressLineIndex() >= 0) ? getAddressLine(0) : "", (getMaxAddressLineIndex() >= 1) ? "\n" + getAddressLine(1) : "", (getMaxAddressLineIndex() >= 2) ? "\n" + getAddressLine(2) : "");
	}
	...
	public boolean isBlank() {
		return (getFeatureName().equals("") && getThoroughfare().equals("") && getLocality().equals("") && getAdminArea().equals("") && getPostalCode().equals(""));
	}
}
