...
public class Task implements Parcelable {
	...
	public Task(Context ctx, long id) throws ParseException {
		this(ctx, ctx.getContentResolver().query(Uri.withAppendedPath(PoolPalContent.TASKS_CONTENT_URI, Long.toString(id)), TaskTable.columnProjection(), null, null, null));
	}
	...
	public String getRepititionString(Context context) {
		...
		return String.format("Every %1$s @ %2$s", context.getResources().getQuantityString(plural_res, getRepitition()), PoolPal.TIME_FORMAT.format(getDate()));
	}
	...
	public Bundle getAddressBundle() {
		...
		Bundle b = new Bundle();
		b.putParcelable(TaskDataAdapter.VALUE_ADDRESS, (hasAddress()) ? PoolAddress.setLines(address) : PoolAddress.blankAddress());
	...
	private Task(Parcel in) throws ParseException {
		...
		this.reminders = new ArrayList<>(Arrays.asList((Reminder[]) in.readParcelableArray(Reminder.class.getClassLoader())));
